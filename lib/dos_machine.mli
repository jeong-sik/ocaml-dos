...
(** 게스트가 키를 물었는데 링이 비어 있었던 횟수. 늘기만 한다.
    ...
    묻는 길은 둘이다. INT 16h(와 INT 21h 콘솔 입력)를 부르거나, BDA 링의
    머리 워드(0040:001A)를 직접 읽어 꼬리와 견준다. 키보드를 직접 읽는
    게임(삼국지3 의 카피프로텍션·이름 입력)은 뒤쪽만 쓴다. 머리를 읽은
    순간 링이 비어 있으면 똑같이 한 번 센다.
